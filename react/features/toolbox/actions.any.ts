...
import { _SET_AUDIOMODE_IS_SPEAKER_ON } from "../mobile/audio-mode/actionTypes";
import {
...
export function setAudioModeSpeaker(isSpeakerOn: boolean) {
    return (dispatch: IStore["dispatch"], getState: IStore["getState"]) => {
        const state = getState();

        const devices = state["features/mobile/audio-mode"]?.devices ?? [];

        console.log("[SpeakerToggle] Available audio devices:", devices);
        if (devices.length < 2) {
            console.warn("[SpeakerToggle] Only one audio device available. Skipping switch.");
            return;
        }

        const targetType = isSpeakerOn ? "SPEAKER" : "EARPIECE";
        const targetDevice = devices.find((d) => d.type === targetType);

        if (targetDevice) {
            console.log(`[SpeakerToggle] Switching to ${targetType} device:`, targetDevice);
            AudioMode.setAudioDevice(targetDevice.uid || targetDevice.type);
            dispatch({
                type: _SET_AUDIOMODE_IS_SPEAKER_ON,
                isSpeakerOn: isSpeakerOn,
            });
        } else {
            console.warn(`[SpeakerToggle] Device not found: ${targetType}`);
        }
    };
}
